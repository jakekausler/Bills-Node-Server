...
export class TaxCalculator {
  ...
  private async calculateWithdrawalTaxes(context: TaxCalculationContext): Promise<TaxCalculationResult> {
    ...
    for (const activity of activities.filter(a => a != null)) {
      if (activity && activity.date && activity.date >= taxYearStart && activity.date <= taxYearEnd) {
        ...
        if (activity.id && typeof activity.id === 'string' && (activity.id.startsWith('AUTO-PULL') || activity.id.startsWith('RMD'))) {
    ...
    if (totalWithdrawalTax > 0) {
      taxActivities.push(this.createTaxActivity(
        account.id,
        'Withdrawal Tax',
        -totalWithdrawalTax,
        calculationDate,
  ...
  private async calculateInterestTaxes(context: TaxCalculationContext): Promise<TaxCalculationResult> {
    ...
    for (const activity of activities.filter(a => a != null)) {
      if (activity && activity.date && activity.date >= taxYearStart && activity.date <= taxYearEnd) {
        ...
        if (activity.id && typeof activity.id === 'string' && activity.id.startsWith('INTEREST-') && (activity.amount as number) > 0) {
  ...
  getTaxSummaryForYear(taxYear: number): {
    ...
    const earlyWithdrawalPenalties = Array.from(accumulation.earlyWithdrawalPenalties.values()).reduce((sum, amount) => sum + amount, 0);
  ...
  getStats(): {
    ...
    const totalEvents = Array.from(this.taxAccumulations.values())
      .reduce((sum, acc) => sum + acc.taxEvents.length, 0);
