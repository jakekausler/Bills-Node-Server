 ...
 * ```
 */
export function loadVariables(simulation) {
    const variables = {};
 ...
 * ```
 */
export function saveVariables(simulations) {
    ...
    for (const variable of [...allVariables].sort()) {
        const row = [variable];
        for (const simulation of simulations) {
            if (typeof simulation.variables[variable].value === 'string' ||
                typeof simulation.variables[variable].value === 'number') {
                row.push(simulation.variables[variable].value.toString());
            }
            else {
                row.push(formatDate(simulation.variables[variable].value));
            }
        }
        stream.write(row);
    }
