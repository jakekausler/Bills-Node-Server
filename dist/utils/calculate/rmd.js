...
dayjs.extend(utc);
export function performRMD(accountsAndTransfers, currDate, balanceMap, idxMap) {
...
function loadRMD() {
    if (Object.keys(RMD_TABLE).length === 0) {
        setRMDTable(Object.fromEntries(Object.entries(load('rmd.json')).map(([k, v]) => [parseInt(k), v])));
    }
}
function rmd(balance, age) {
    loadRMD();
    if (age in RMD_TABLE) {
