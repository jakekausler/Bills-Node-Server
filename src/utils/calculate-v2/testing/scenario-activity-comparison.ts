...
function compareScenario(scenarioName: string): ComparisonResult[] {
  ...
  const calculatedData: Record<string, CalculatedAccount> = JSON.parse(
    fs.readFileSync(calculatedPath, 'utf8')
  );

  // Load original responses
  const originalResponses: OriginalResponse[] = JSON.parse(
    fs.readFileSync(originalPath, 'utf8')
  );

  // Filter original responses for this scenario
  const scenarioOriginals = originalResponses.filter(response => 
...
function printResults(results: ComparisonResult[], detailed: boolean = false): void {
  ...
  for (const result of results) {
    ...
    const accuracy = result.totalOriginal > 0 ? 
      (result.exactMatches / result.totalOriginal * 100).toFixed(1) : '0.0';
  ...
  // Summary
  const overallAccuracy = totalOriginal > 0 ? 
    (totalMatches / totalOriginal * 100).toFixed(1) : '0.0';
...
function main() {
  ...
  if (!scenarioName) {
    ...
    console.error('Available scenarios: historical_to_current, current_to_near_future, recent_to_medium_term, extended_projection, far_future_projection');
    process.exit(1);
  }
