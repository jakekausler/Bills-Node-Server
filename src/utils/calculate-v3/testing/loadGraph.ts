...
async function loadJakeGraphData() {
  ...
  const graphData = loadGraph({ accounts: [jakeAccount], transfers: { activity: [], bills: [] } }, startDate, endDate);

  writeFileSync('testing/loadGraph/jake-graph-july-2025.json', JSON.stringify(graphData, null, 2));
