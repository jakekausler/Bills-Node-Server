...
import { compoundInterest } from '../../data/interest/interest';
import { nextDate } from './helpers';
import { v4 as uuidv4 } from 'uuid';
import { load } from '../io/io';
dayjs.extend(utc);
export function payInterestTaxes(accountsAndTransfers, currDate, balanceMap, idxMap) {
        ...
        if (account.interestPayAccount) {
            ...
            for (let i = account.consolidatedActivity.length - 1; i >= 0; i--) {
                ...
                if (activity.category === 'Banking.Interest') {
                    ...
                    taxableMap[account.id].amount += activity.amount * (account.interestTaxRate || 0);
                }
            }
        }
    }
    Object.entries(taxableMap).forEach(([accountId, { payee, amount }]) => {
        if (amount > 0) {
            ...
            const payeeAccount = getById(accountsAndTransfers.accounts, payee);
            if (!payeeAccount) {
...
export function handleInterest(account, currDate, simulation, interestIdxMap, interestMap, nextInterestMap, balanceMap, idxMap, monteCarlo) {
    // Nothing to do if there is no interest
    if (!interestMap[account.id])
        return;
    if (
    ...
    if (isSame(nextInterestMap[account.id], currDate)) {
        ...
        const activity = interestMap[account.id].toActivity(uuidv4(), simulation, compoundInterest(balanceMap[account.id], getRate(currDate.getFullYear(), interestMap[account.id]?.apr, interestMap[account.id]?.aprVariable, monteCarlo), interestMap[account.id]?.compounded), currDate);
        activity.flag = true;
        activity.flagColor = activity.amount < 0 ? 'pink' : 'lime';
        ...
        if (activity.amount !== 0) {
            ...
            const interest = new ConsolidatedActivity(activity.serialize(), {
                interestId: interestMap[account.id].id,
            });
            // if (account.name === 'Mortgage') {
            ...
            if (isSame(interestMap[account.id]?.applicableDate, currDate)) {
                account.consolidatedActivity[idxMap[account.id]].firstInterest = true;
            }
            // Update the balance with the interest's amount
            balanceMap[account.id] += activity.amount;
            account.consolidatedActivity[idxMap[account.id]].balance = balanceMap[account.id];
            idxMap[account.id] += 1;
        }
        nextInterestMap[account.id] = nextDate(nextInterestMap[account.id], interestMap[account.id].compounded, 1);
    }
}
let RATES = null;
let PORTFOLIO = null;
let YEAR_RATES = null;
let YEAR_INVESTMENT_RATES = null;
export function loadRatesToYears(startYear, endYear) {
    ...
    for (let year = startYear; year <= endYear; year++) {
        ...
        YEAR_RATES[year] = {
            HIGH_YIELD_SAVINGS_RATE: RATES.savings.highYield[Math.floor(Math.random() * RATES.savings.highYield.length)] / 100,
...
function setYearInvestmentRates(year) {
    if (!RATES) {
        throw new Error('Rates not loaded');
    }
...
function calculateInvestmentRate(year) {
    if (!RATES) {
        throw new Error('Rates not loaded');
    }
    if (!PORTFOLIO) {
...
function getRate(year, apr, aprVariable, monteCarlo) {
