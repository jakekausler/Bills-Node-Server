...
export class Bill {
     ...
     * @private
     */
    initializeBasicProperties(data) {
     ...
     * @private
     */
    initializeTransferProperties(data) {
     ...
     * @private
     */
    initializeSchedulingProperties(data) {
     ...
     * @private
     */
    initializeInflationProperties(data, simulation) {
        try {
            const { amount: increaseBy, amountIsVariable: increaseByIsVariable, amountVariable: increaseByVariable, } = loadNumberOrVariable(data.increaseBy, data.increaseByIsVariable, data.increaseByVariable, simulation);
            this.increaseBy = [true, false].includes(increaseByIsVariable) ? increaseBy : 0.03;
     ...
     * @private
     */
    initializeAnnualDateProperties(data) {
     ...
     * @private
     */
    initializeDateProperties(data, simulation) {
        const { date: startDate, dateIsVariable: startDateIsVariable, dateVariable: startDateVariable, } = loadDateOrVariable(data.startDate, data.startDateIsVariable, data.startDateVariable, simulation);
        this.startDate = startDate;
        this.startDateIsVariable = startDateIsVariable;
        this.startDateVariable = startDateVariable;
        if (data.endDate) {
            const { date: endDate, dateIsVariable: endDateIsVariable, dateVariable: endDateVariable, } = loadDateOrVariable(data.endDate, data.endDateIsVariable, data.endDateVariable, simulation);
     ...
     * @private
     */
    initializeAmountProperties(data, simulation) {
     ...
     */
    setIncreaseByDate(increaseByDate) {
     ...
     */
    toActivity(id, simulation, amount, date) {
        return new Activity({
            id: id,
            name: this.name,
            category: this.category,
 ...
 */
export function insertBill(accountsAndTransfers, account, bill, data, isTransfer, simulation = 'Default') {
