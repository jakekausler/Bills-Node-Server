...
export function handleSocialSecurity(accountsAndTransfers, socialSecurities, currDate, balanceMap, idxMap) {
    socialSecurities.forEach((socialSecurity) => {
        ...
        if (isAfterOrSame(currDate, socialSecurity.startDate) &&
...
function calculateAIME(accountsAndTransfers, socialSecurity) {
...
function getIndexedAnnualIncomes(yearTurn60, yearlyIncomes) {
    const averageWageIndex = getAverageWageIndex(yearTurn60);
    const indexedAnnualIncomes = [];
...
function getAverageWageIndex(yearTurn60) {
...
function getBendPoints(yearTurns62) {
    ...
    const years = Object.keys(bendPoints)
        .map((x) => parseInt(x))
        .sort((a, b) => a - b);
    const firstBendPointIncreases = [];
    const secondBendPointIncreases = [];
    for (let i = 1; i < years.length; i++) {
        const year = years[i];
        const prevYear = years[i - 1];
    ...
    const firstBendPointAverageIncrease = firstBendPointIncreases.reduce((sum, val) => sum + val, 0) / firstBendPointIncreases.length;
    const secondBendPointAverageIncrease = secondBendPointIncreases.reduce((sum, val) => sum + val, 0) / secondBendPointIncreases.length;
...
function computePIA(yearTurns62, aime) {
...
function factorForCollectionAge(collectionAge) {
    if (collectionAge < 62) {
        return 0;
    }
    if (collectionAge === 62) {
...
function computeMonthlyBenefit(accountsAndTransfers, socialSecurity) {
