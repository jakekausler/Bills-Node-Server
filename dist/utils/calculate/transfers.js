...
export function addTransfers(account, endDate, simulation, transfers, monteCarlo) {
...
export function dealWithOtherTransfers(account, accounts, idxMap, balanceMap) {
    ...
    if (activity.isTransfer &&
        ...
        const otherActivity = getById(otherAccount.consolidatedActivity, activity.id);
        if (!otherActivity) {
            throw new Error(`Activity ${activity.id} not found on other side`);
        }
        let adjustedAmount = activity.amount;
...
export function dealWithSpecialFractions(account, accounts, idxMap, balanceMap) {
    ...
    if (account.consolidatedActivity[idxMap[account.id]].amount === '{HALF}' ||
        ...
        const balanceToUse = otherAccount.type === 'Loan' || otherAccount.type === 'Credit'
        ...
        const { item: otherActivity, idx: otherActivityIdx } = getByIdWithIdx(otherAccount.consolidatedActivity, account.consolidatedActivity[idxMap[account.id]].id);
        if (!otherActivity) {
