...
async function loadOriginalResponses(): Promise<OriginalResponse[]> {
  const responsesDir = path.join(process.cwd(), 'src/utils/calculate-v2/original-responses');
...
async function runCalculateV2(
  ...
  const testData: AccountsAndTransfers = {
    accounts: accountsAndTransfers.accounts.map(account => new Account({
      ...account,
...
function compareResults(
  ...
  const percentageDiff = originalFinalBalance !== 0 ? 
    Math.abs(balanceDiff / originalFinalBalance) * 100 : 
    (balanceDiff !== 0 ? 100 : 0);
  ...
  const significant = Math.abs(activityCountDiff) > 0 || 
    (shouldCompareBalance && Math.abs(balanceDiff) > 0.01);
...
export async function runAccuracyComparison(): Promise<void> {
  ...
  try {
    ...
    console.log(`📊 Testing ${TEST_SCENARIOS.length} scenarios with ${accountsAndTransfers.accounts.length} accounts\n`);
    ...
    for (let scenarioIndex = 0; scenarioIndex < TEST_SCENARIOS.length; scenarioIndex++) {
      ...
      try {
        ...
        const calculatedResults = await runCalculateV2(
        ...
        if (scenario.id === 'current_to_near_future') {
          try {
            mkdirSync('./calculated-activities', { recursive: true });
            const calculatedActivitiesFile = `./calculated-activities/${scenario.id}.json`;
        ...
        for (const originalResponse of scenarioOriginals) {
          ...
          if (comparison.success && !comparison.differences.significant) {
            scenarioSuccesses++;
            console.log(`    ✅ ${comparison.accountName}: Match (${comparison.calculated.activityCount} activities, $${comparison.calculated.finalBalance.toFixed(2)})`);
          } else if (comparison.differences.significant) {
            scenarioDiscrepancies++;
            console.log(`    ⚠️  ${comparison.accountName}: Discrepancy`);
            console.log(`       Activity count: ${comparison.original.activityCount} → ${comparison.calculated.activityCount} (${comparison.differences.activityCountDiff >= 0 ? '+' : ''}${comparison.differences.activityCountDiff})`);
            console.log(`       Final balance: $${comparison.original.finalBalance.toFixed(2)} → $${comparison.calculated.finalBalance.toFixed(2)} (${comparison.differences.balanceDiff >= 0 ? '+' : ''}$${comparison.differences.balanceDiff.toFixed(2)})`);
    ...
    console.log(`  Successful: ${summary.successfulComparisons} (${(summary.successfulComparisons / summary.totalComparisons * 100).toFixed(1)}%)`);
    console.log(`  Failed: ${summary.failedComparisons} (${(summary.failedComparisons / summary.totalComparisons * 100).toFixed(1)}%)`);
    console.log(`  Significant discrepancies: ${summary.significantDiscrepancies} (${(summary.significantDiscrepancies / summary.totalComparisons * 100).toFixed(1)}%)`);
    ...
    for (const scenarioSummary of summary.scenarios) {
      ...
      console.log(`    Successful: ${scenarioSummary.successful} (${(scenarioSummary.successful / scenarioSummary.comparisons * 100).toFixed(1)}%)`);
      console.log(`    Discrepancies: ${scenarioSummary.discrepancies} (${(scenarioSummary.discrepancies / scenarioSummary.comparisons * 100).toFixed(1)}%)`);
    ...
    const outputDir = path.join(process.cwd(), 'src/utils/calculate-v2/comparison-results');
    ...
    if (summary.significantDiscrepancies === 0) {
      ...
      console.log(`⚠️  MINOR DISCREPANCIES - ${summary.significantDiscrepancies} out of ${summary.totalComparisons} comparisons show differences`);
      console.log('🔧 Review discrepancies for production readiness');
    } else {
      console.log(`❌ SIGNIFICANT DISCREPANCIES - ${summary.significantDiscrepancies} out of ${summary.totalComparisons} comparisons show differences`);
...
function generateComparisonSummary(results: ComparisonResult[]): ComparisonSummary {
  ...
  const scenarios = TEST_SCENARIOS.map(scenario => {
    ...
    const avgBalanceDiff = scenarioResults.length > 0 ?
      scenarioResults.reduce((sum, r) => sum + Math.abs(r.differences.balanceDiff), 0) / scenarioResults.length : 0;
    const avgActivityCountDiff = scenarioResults.length > 0 ?
      scenarioResults.reduce((sum, r) => sum + Math.abs(r.differences.activityCountDiff), 0) / scenarioResults.length : 0;
