...
describe('Push/Pull Event Processing', () => {
  ...
  it('should find accounts with push/pull configurations', () => {
    const pushPullAccounts = testData.accounts.filter((acc) => acc.performsPulls || acc.performsPushes);
  ...
  it('should process push/pull events without errors', async () => {
    const result = await calculateAllActivity(testData, new Date('2025-06-01'), new Date('2025-08-01'), 'Default');
  ...
  it('should generate push/pull activities when conditions are met', async () => {
    ...
    for (const account of result.accounts) {
      const pushPullActivities = account.consolidatedActivity.filter(
        (act) => act.name?.includes('Auto Push') || act.name?.includes('Auto Pull'),
      );
